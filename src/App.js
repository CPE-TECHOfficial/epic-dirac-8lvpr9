...
const INITIAL_SETTINGS = {
  ...
  showTicker: true,
  tickerText: "🔥 عروض خاصة: خصم 20% على باقات الحماية المتكاملة! • 🟢 المركز مفتوح الآن لاستقبال طلباتكم.",
};

const INITIAL_STORE_ITEMS = [
  { id: 1, name: "شاحن Anker 20W الأصلي", category: "شواحن", price: 25000, badge: "أصلي", location: "الكوت - المركز", image: "https://images.unsplash.com/photo-1620023490075-d4c38258e2d4?w=500&q=80" },
  { id: 2, name: "سماعة AirPods Pro Copy", category: "سماعات", price: 35000, badge: "الأكثر طلباً", location: "بغداد - الكرادة", image: "https://images.unsplash.com/photo-1603351154351-5cf99bc32f2d?w=500&q=80" },
  { id: 3, name: "كابل آيفون قماشي", category: "شواحن", price: 15000, badge: "ضمان سنة", location: "الكوت - الهورة", image: "https://images.unsplash.com/photo-1585856407008-011400477209?w=500&q=80" },
  { id: 4, name: "بكج حماية متكامل", category: "حماية", price: 15000, badge: "عرض خاص", location: "الكوت", image: "https://images.unsplash.com/photo-1616348436168-de43ad0db179?w=500&q=80" },
];
...
function SalesPopup() {
  ...
  useEffect(() => {
    const messages = ["تم طلب شاشة iPhone 13", "تم إصلاح لابتوب Dell", "أحمد اشترى شاحن Anker"];
  ...
  return (
    <div className="fixed bottom-20 left-4 bg-slate-800 border border-cyan-500/30 p-4 rounded-xl shadow-2xl z-50 animate-in slide-in-from-left flex items-center gap-3">
      ...
      <div><p className="text-white text-sm font-bold">نشاط حديث</p><p className="text-slate-300 text-xs">{msg}</p></div>
    </div>
  );
}
...
function BeforeAfter() {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-64 md:h-96 rounded-2xl overflow-hidden cursor-ew-resize border border-white/10 group select-none" onMouseMove={handleMove} onTouchMove={handleMove}>
      ...
      <div className="absolute inset-0 bg-black/50 flex items-center justify-center pointer-events-none"><span className="bg-green-600/80 text-white px-3 py-1 rounded font-bold text-sm">بعد الصيانة</span></div>
      <div className="absolute inset-0 w-full h-full overflow-hidden border-r-2 border-white" style={{ width: `${sliderPos}%` }}>
        ...
        <div className="absolute inset-0 bg-black/10 flex items-center justify-center pointer-events-none"><span className="bg-red-600/80 text-white px-3 py-1 rounded font-bold text-sm">قبل (مكسور)</span></div>
      </div>
...
function CostEstimator() {
  ...
  const getPrice = () => {
    if (device === "iphone" && issue === "screen") return "50,000 - 150,000 د.ع";
    if (device === "iphone" && issue === "battery") return "35,000 - 60,000 د.ع";
    if (device === "android" && issue === "screen") return "30,000 - 100,000 د.ع";
    if (issue === "software") return "10,000 - 25,000 د.ع";
    return "يرجى الفحص";
  };
  return (
    <div className="bg-slate-800 p-6 rounded-2xl border border-white/5 mt-8">
      <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Calculator className="text-yellow-400" /> حاسبة التكلفة التقديرية</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <select onChange={(e) => setDevice(e.target.value)} className="bg-slate-900 text-white p-3 rounded-lg border border-white/10"><option value="iphone">آيفون (iPhone)</option><option value="android">أندرويد (Samsung/Huawei)</option></select>
        <select onChange={(e) => setIssue(e.target.value)} className="bg-slate-900 text-white p-3 rounded-lg border border-white/10"><option value="screen">كسر شاشة</option><option value="battery">ضعف بطارية</option><option value="software">سوفت وير</option></select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div onClick={() => onNavigate("store")} className="h-40 rounded-2xl relative overflow-hidden group cursor-pointer border border-white/10 shadow-2xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-purple-900/90 to-transparent flex flex-col justify-end p-6">
            <h3 className="text-2xl font-bold text-white flex items-center gap-2"><ShoppingBag className="text-purple-400" /> المتجر</h3>
          </div>
        </div>
        <div onClick={() => onNavigate("tracking")} className="h-40 rounded-2xl relative overflow-hidden group cursor-pointer border border-white/10 shadow-2xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-blue-900/90 to-transparent flex flex-col justify-end p-6">
            <h3 className="text-2xl font-bold text-white flex items-center gap-2"><Activity className="text-blue-400" /> تتبع الطلب</h3>
          </div>
        </div>
        <div onClick={() => onNavigate("downloads")} className="h-40 rounded-2xl relative overflow-hidden group cursor-pointer border border-white/10 shadow-2xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-orange-900/90 to-transparent flex flex-col justify-end p-6">
            <h3 className="text-2xl font-bold text-white flex items-center gap-2"><Download className="text-orange-400" /> التحميلات</h3>
          </div>
        </div>
      </div>
    </div>
...
function AdminPanel({ isOpen, onClose, data, handlers }) {
  ...
  const [newItem, setNewItem] = useState({ name: "", price: "", category: "عام", location: "", image: "" });
  ...
  const handleAddItem = () => {
    ...
    setNewItem({ name: "", price: "", category: "عام", location: "", image: "" });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-in fade-in">
      <div className="bg-slate-900 w-full max-w-5xl rounded-2xl border border-white/20 h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-800">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Settings className="text-cyan-400" /> لوحة الإدارة</h2>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-1/4 bg-slate-950 border-l border-white/10 p-4 space-y-2 hidden md:block">
            <button onClick={() => setActiveTab("store")} className={`w-full text-right p-3 rounded-xl font-bold flex items-center gap-2 ${activeTab === "store" ? "bg-cyan-600 text-white" : "text-slate-400 hover:bg-white/5"}`}><ShoppingBag size={18}/> إدارة المتجر</button>
            <button onClick={() => setActiveTab("downloads")} className={`w-full text-right p-3 rounded-xl font-bold flex items-center gap-2 ${activeTab === "downloads" ? "bg-cyan-600 text-white" : "text-slate-400 hover:bg-white/5"}`}><Download size={18}/> ملفات الصيانة</button>
            <button onClick={() => setActiveTab("social")} className={`w-full text-right p-3 rounded-xl font-bold flex items-center gap-2 ${activeTab === "social" ? "bg-cyan-600 text-white" : "text-slate-400 hover:bg-white/5"}`}><Share2 size={18}/> النشر التلقائي</button>
            <button onClick={() => setActiveTab("settings")} className={`w-full text-right p-3 rounded-xl font-bold flex items-center gap-2 ${activeTab === "settings" ? "bg-cyan-600 text-white" : "text-slate-400 hover:bg-white/5"}`}><Key size={18}/> الإعدادات</button>
          </div>
          <div className="flex-1 p-6 overflow-y-auto bg-slate-900">
            {activeTab === "store" && (
              <div className="space-y-8">
                <div className="bg-slate-800 p-6 rounded-xl border border-white/10">
                  <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Plus className="text-green-400"/> إضافة منتج</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" placeholder="اسم المنتج" value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} className="bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                    <input type="number" placeholder="السعر" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} className="bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                    <input type="text" placeholder="القسم" value={newItem.category} onChange={e => setNewItem({...newItem, category: e.target.value})} className="bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                    <input type="text" placeholder="رابط الصورة" value={newItem.image} onChange={e => setNewItem({...newItem, image: e.target.value})} className="bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                    <input type="text" placeholder="الموقع" value={newItem.location} onChange={e => setNewItem({...newItem, location: e.target.value})} className="bg-black/30 text-white p-3 rounded-lg border border-white/10 md:col-span-2" />
                  </div>
                  <button onClick={handleAddItem} className="mt-4 w-full bg-green-600 hover:bg-green-500 text-white p-3 rounded-lg font-bold">نشر</button>
                </div>
            ...
            {activeTab === "downloads" && (
              <div className="space-y-6">
                 <div className="bg-slate-800 p-6 rounded-xl border border-white/10">
                  <h3 className="text-xl font-bold text-white mb-4">إضافة ملف</h3>
                  <div className="flex gap-2">
                    <input type="text" placeholder="اسم البرنامج" value={newDownload.name} onChange={e => setNewDownload({...newDownload, name: e.target.value})} className="flex-1 bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                    <input type="text" placeholder="الحجم" value={newDownload.size} onChange={e => setNewDownload({...newDownload, size: e.target.value})} className="w-24 bg-black/30 text-white p-3 rounded-lg border border-white/10" />
                  </div>
                  <button onClick={handleAddDownload} className="mt-4 w-full bg-blue-600 text-white p-3 rounded-lg font-bold">إضافة</button>
                </div>
            ...
            {activeTab === "social" && (
              <div className="flex flex-col h-full justify-center items-center text-center space-y-6">
                <div className="bg-slate-800 p-8 rounded-2xl border border-white/10 w-full max-w-lg">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2">النشر المتزامن</h3>
                  <textarea value={socialPost} onChange={e => setSocialPost(e.target.value)} placeholder="اكتب المنشور..." className="w-full h-32 bg-black/30 text-white p-4 rounded-xl border border-white/10 mb-4 focus:border-blue-500 outline-none"></textarea>
                  <button onClick={handleSocialPublish} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white py-3 rounded-xl font-bold hover:shadow-lg hover:shadow-blue-500/20 transition">نشر الآن 🚀</button>
                </div>
              </div>
            )}
            {activeTab === "settings" && (
              <div className="space-y-6">
                <div className="bg-slate-800 p-6 rounded-xl border border-white/10">
                  <h3 className="text-xl font-bold text-white mb-4">بيانات الدخول</h3>
                  ...
                  <input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="كلمة المرور الجديدة" className="w-full bg-black/30 text-white p-3 rounded-lg border border-white/10 mb-4" />
                  <button onClick={handleUpdateCreds} className="bg-purple-600 text-white px-6 py-3 rounded-lg font-bold">حفظ</button>
                </div>
              </div>
            )}
          </div>
    ...
    <div className="min-h-screen bg-[#0a0f1c] pb-20 animate-in fade-in">
      <div className="bg-slate-900 border-b border-white/10 px-6 py-6 sticky top-0 z-40 shadow-xl">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white font-bold"><ArrowRight size={18} /> الرئيسية</button>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><ShoppingBag className="text-purple-400"/> المتجر</h1>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="flex gap-3 overflow-x-auto pb-4 mb-4 scrollbar-hide">
          {categories.map(cat => (<button key={cat} onClick={() => setFilter(cat)} className={`px-5 py-2 rounded-full font-bold whitespace-nowrap transition-all ${filter === cat ? "bg-purple-600 text-white" : "bg-slate-800 text-slate-400"}`}>{cat === "all" ? "الكل" : cat}</button>))}
        </div>
          ...
          {filtered.map(item => (
            <div key={item.id} className="bg-slate-800 rounded-xl overflow-hidden border border-white/5 hover:border-purple-500/50 transition-all group flex flex-col">
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto flex justify-between items-center pt-3 border-t border-white/10"><span className="font-bold text-xl text-purple-400">{item.price.toLocaleString()} د.ع</span><button onClick={() => window.open(`https://wa.me/${CONTACT_INFO.rawPhone}?text=أريد شراء ${item.name}`, "_blank")} className="bg-purple-600 hover:bg-purple-500 text-white p-2 rounded-lg transition"><MessageCircle size={20}/></button></div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] pb-20 animate-in fade-in">
      <div className="bg-slate-900 border-b border-white/10 px-6 py-8">
        <div className="max-w-4xl mx-auto"><button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white mb-6"><ArrowRight size={18} /> الرئيسية</button><h1 className="text-3xl font-bold text-white flex items-center gap-3"><Download className="text-orange-400" /> منطقة الفنيين</h1></div>
      </div>
      <div className="max-w-4xl mx-auto px-6 mt-8 space-y-4">
        {items.map((file) => (
          <div key={file.id} className="bg-slate-800 p-4 rounded-xl border border-white/5 flex justify-between items-center hover:bg-slate-700/50 transition-colors">
            <div className="flex items-center gap-4"><div className="bg-orange-500/20 p-3 rounded-lg"><FileText className="text-orange-400" /></div><div><h3 className="text-white font-bold">{file.name}</h3><p className="text-slate-400 text-sm">{file.type} • {file.size}</p></div></div><a href={file.url} className="bg-white/10 hover:bg-orange-500 text-white px-4 py-2 rounded-lg font-bold transition-colors text-sm">تحميل</a>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0a0f1c] animate-in slide-in-from-bottom-5 duration-500 pb-20">
      <div className="bg-gradient-to-r from-slate-900 to-blue-950 px-6 py-12 border-b border-white/10 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          <button onClick={onBack} className="flex items-center gap-2 text-cyan-400 hover:text-white mb-6 font-bold bg-white/5 w-fit px-4 py-2 rounded-lg"><ArrowRight size={18} /> الرئيسية</button>
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">قسم السوفت وير</h1>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-6 mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-slate-800/50 border border-white/5 rounded-2xl p-8 hover:border-cyan-500/30 transition-all">
          <div className="flex items-center gap-3 mb-6"><ShieldCheck className="text-green-400 w-8 h-8" /><h2 className="text-2xl font-bold text-white">تخطي جوجل (FRP)</h2></div>
          <button onClick={onContact} className="mt-8 w-full bg-white/5 hover:bg-green-600 hover:text-white text-slate-300 py-3 rounded-xl transition-all font-bold border border-white/10">طلب الخدمة</button>
        </div>
        <div className="bg-slate-800/50 border border-white/5 rounded-2xl p-8 hover:border-blue-500/30 transition-all">
          <div className="flex items-center gap-3 mb-6"><Unlock className="text-blue-400 w-8 h-8" /><h2 className="text-2xl font-bold text-white">فك شفرات</h2></div>
          <button onClick={onContact} className="mt-8 w-full bg-white/5 hover:bg-blue-600 hover:text-white text-slate-300 py-3 rounded-xl transition-all font-bold border border-white/10">استفسار</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f0f10] animate-in slide-in-from-bottom-5 duration-500 pb-20">
      <div className="bg-gradient-to-r from-slate-900 to-emerald-950 px-6 py-12 border-b border-white/10">
        <div className="max-w-7xl mx-auto">
          <button onClick={onBack} className="flex items-center gap-2 text-emerald-400 hover:text-white mb-6 font-bold bg-white/5 w-fit px-4 py-2 rounded-lg"><ArrowRight size={18} /> الرئيسية</button>
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">صيانة الهاردوير</h1>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-6 mt-12 space-y-12">
        <div className="bg-slate-900 rounded-2xl p-6 border border-white/5 text-center"><h3 className="text-2xl font-bold text-white mb-6">شاهد الفرق في الجودة</h3><BeforeAfter /></div>
      </div>
    </div>
  );
}
...
function PricingView({ onBack, onContact }) {
  return (
    <div className="min-h-screen bg-slate-950 animate-in fade-in duration-500 pb-20">
      <div className="bg-slate-900 border-b border-white/10 px-6 py-8"><div className="max-w-4xl mx-auto"><button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white mb-6"><ArrowRight size={18} /> رجوع</button><h1 className="text-3xl font-bold text-white flex items-center gap-3"><Banknote className="text-green-400" /> الأسعار التقريبية</h1></div></div>
      <div className="max-w-4xl mx-auto px-6 mt-8">
        ...
        <button onClick={onContact} className="w-full mt-6 bg-green-600 hover:bg-green-500 text-white py-4 rounded-xl font-bold text-lg">طلب تسعير خاص</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
      <div className="bg-slate-900 rounded-2xl w-full max-w-md border border-white/20 p-6 relative shadow-2xl">
        ...
        <h2 className="text-2xl font-bold text-white mb-6 text-center">تتبع حالة الجهاز</h2>
        <div className="flex gap-2 mb-6"><input type="text" value={code} onChange={(e) => setCode(e.target.value)} placeholder="رقم الوصل" className="flex-1 bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-white text-center" /><button onClick={checkOrder} className="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold">فحص</button></div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
      <div className="bg-slate-900 rounded-2xl w-full max-w-md border border-white/20 p-6 relative">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X /></button>
        <h2 className="text-2xl font-bold text-white mb-2 text-center">حجز موعد صيانة</h2>
        <div className="space-y-4 mt-4">
            <input type="text" placeholder="نوع الجهاز" className="w-full bg-slate-800 border border-white/10 rounded-xl p-3 text-white" />
            <input type="text" placeholder="وصف العطل" className="w-full bg-slate-800 border border-white/10 rounded-xl p-3 text-white" />
            <input type="date" className="w-full bg-slate-800 border border-white/10 rounded-xl p-3 text-white" />
            <button className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold">إرسال الحجز</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const handleLogin = (u, p) => {
    if (u === adminCreds.user && p === adminCreds.pass) { setIsLoggedIn(true); setShowLoginModal(false); setShowAdminPanel(true); } else { alert("بيانات الدخول غير صحيحة"); }
  };
  ...
  if (currentView === "software") return <SoftwareView onBack={() => setCurrentView("home")} onContact={() => alert('تواصل معنا على واتساب')} />;
  if (currentView === "hardware") return <HardwareView onBack={() => setCurrentView("home")} onContact={() => alert('تواصل معنا على واتساب')} />;
  if (currentView === "pricing") return <PricingView onBack={() => setCurrentView("home")} onContact={() => alert('تواصل معنا على واتساب')} />;
  if (currentView === "tracking") return <div className="min-h-screen bg-slate-900 p-8"><button onClick={() => setCurrentView("home")} className="text-white mb-4">رجوع</button><TrackingModal isOpen={true} onClose={() => setCurrentView("home")} /></div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 font-sans text-right" dir="rtl">
      ...
      <header className="px-6 py-4 sticky top-0 z-30 bg-slate-900/90 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             <button onClick={() => setCurrentView("store")} className="hidden md:flex items-center gap-2 text-purple-400 font-bold px-3 py-2 bg-purple-500/10 rounded-lg hover:bg-purple-500/20"><ShoppingBag size={18} /> المتجر</button>
             <button onClick={() => setCurrentView("pricing")} className="hidden md:flex items-center gap-2 text-green-400 font-bold px-3 py-2 bg-green-500/10 rounded-lg hover:bg-green-500/20"><Banknote size={18} /> الأسعار</button>
            {isLoggedIn ? (
      ...
      <section className="relative h-[500px] flex items-center justify-center bg-black overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4">
          <h2 className="text-5xl md:text-6xl font-extrabold text-white mb-6">المركز الأول للحلول التقنية</h2>
          <p className="text-xl text-slate-300 mb-8 max-w-2xl mx-auto">صيانة هاردوير احترافية • برمجة وتخطي • متجر إلكتروني متكامل • أدوات حصرية للفنيين</p>
          <div className="flex flex-wrap justify-center gap-4">
            <button onClick={() => setShowBookingModal(true)} className="bg-[#25D366] text-white px-8 py-4 rounded-xl font-bold hover:scale-105 transition flex items-center gap-2"><Calendar className="w-5 h-5"/> حجز موعد</button>
            <button onClick={() => setCurrentView("downloads")} className="bg-slate-700/80 text-white px-8 py-4 rounded-xl font-bold hover:bg-slate-600 transition flex items-center gap-2"><Download className="w-5 h-5"/> منطقة الفنيين</button>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          <div onClick={() => setCurrentView("hardware")} className="cursor-pointer bg-slate-800/50 p-8 rounded-2xl border border-white/5 hover:border-emerald-500/50 hover:shadow-2xl transition group">
             ...
             <h3 className="text-3xl font-bold text-white mb-2">قسم الهاردوير</h3>
             <p className="text-slate-400 mb-4">صيانة بوردات، تبديل شاشات، إصلاح دوائر الشحن والباور بأحدث الأجهزة.</p>
             <span className="text-emerald-400 font-bold flex items-center gap-2">التفاصيل <ArrowLeft size={16}/></span>
          </div>
          <div onClick={() => setCurrentView("software")} className="cursor-pointer bg-slate-800/50 p-8 rounded-2xl border border-white/5 hover:border-blue-500/50 hover:shadow-2xl transition group">
             ...
             <h3 className="text-3xl font-bold text-white mb-2">قسم السوفت وير</h3>
             <p className="text-slate-400 mb-4">فك تشفير الشبكات، تخطي حسابات جوجل (FRP)، حلول برمجية متقدمة.</p>
             <span className="text-blue-400 font-bold flex items-center gap-2">التفاصيل <ArrowLeft size={16}/></span>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-12 bg-black/30 border-y border-white/5">
        <div className="max-w-7xl mx-auto">
           <h3 className="text-2xl font-bold text-white mb-8 text-center">آراء العملاء</h3>
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-slate-800 p-6 rounded-xl border border-white/5"><div className="flex text-yellow-400 mb-2"><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/></div><p className="text-slate-300 text-sm">"أفضل مركز تعاملت معه في الكوت، شغل نظيف جداً."</p><p className="text-white font-bold text-xs mt-4">- علي محمد</p></div>
              <div className="bg-slate-800 p-6 rounded-xl border border-white/5"><div className="flex text-yellow-400 mb-2"><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/></div><p className="text-slate-300 text-sm">"خدمة سريعة وأسعارهم مناسبة مقارنة بالسوق."</p><p className="text-white font-bold text-xs mt-4">- سارة أحمد</p></div>
              <div className="bg-slate-800 p-6 rounded-xl border border-white/5"><div className="flex text-yellow-400 mb-2"><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14} fill="currentColor"/><Star size={14}/></div><p className="text-slate-300 text-sm">"المتجر يحتوي على منتجات أصلية، اشتريت شاحن وكان ممتاز."</p><p className="text-white font-bold text-xs mt-4">- حسين كاظم</p></div>
           </div>
        </div>
      </section>
      ...
      <footer className="bg-slate-950 py-12 border-t border-white/10 mt-auto">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
           <div>
             ...
             <p className="text-slate-400 text-sm leading-relaxed">الوجهة الأولى في واسط لصيانة الأجهزة الذكية والحواسيب. نقدم حلولاً تقنية متكاملة بضمان حقيقي.</p>
           </div>
           <div>
           ...
           <div><h3 className="text-white font-bold mb-4">الموقع</h3><LocationMap /></div>
        </div>
        <div className="text-center text-slate-600 text-sm mt-8 border-t border-white/5 pt-4">© 2025 CPE-TECH. جميع الحقوق محفوظة.</div>
      </footer>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-800 p-8 rounded-2xl w-full max-w-sm border border-white/10 relative">
             ...
             <h2 className="text-2xl font-bold text-white mb-6 text-center">دخول الإدارة</h2>
             <form onSubmit={(e) => { e.preventDefault(); handleLogin(e.target.u.value, e.target.p.value); }}>
               <input name="u" type="text" placeholder="اسم المستخدم" className="w-full mb-3 p-3 rounded bg-black/30 text-white border border-white/10" />
               <input name="p" type="password" placeholder="كلمة المرور" className="w-full mb-6 p-3 rounded bg-black/30 text-white border border-white/10" />
               <button className="w-full bg-blue-600 text-white p-3 rounded font-bold hover:bg-blue-500">دخول</button>
             </form>
          </div>
        </div>
      )}
